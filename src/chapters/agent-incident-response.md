...
The agent did not malfunction. It acted on an instruction that, under its delegated authority, it was technically permitted to carry out. The failure was in governance: the agent operated with authority it should not have held during a freeze — not explicitly granted for that context, not scoped to current conditions, and not revoked when the situation changed. A traditional incident response playbook looks for the bug. There was no bug. There was a governance gap.
...
**Blast radius assessment requires tracing the delegation chain.** In software incidents, blast radius is bounded by the system's data access scope. In agent incidents, blast radius is bounded by the agent's delegated authority, which may span multiple systems, multiple tool integrations, and multiple downstream agents that acted on this agent's outputs before the incident was detected. Tracing what the agent touched requires following that chain, not scanning a single system's logs.
...
CoSAI's AI Incident Response Framework, adapted from the NIST lifecycle and published in October 2025, identifies agent-specific incident categories across data, model, deployment, infrastructure, and user interaction domains.[^cosai-ir] For every category, CoSAI notes the same pattern: the immediate cause is a specific exploit or failure mode, but the structural cause is almost always insufficient delegation controls. An agent that abuses a tool was given access it should not have had. An agent that follows injected instructions lacked input validation at a trust boundary. An agent that operates outside its intended scope was given credentials that did not constrain that scope.

The governance questions for root cause follow directly from PAC's Accountability pillar:

- **Did the liability chain exist before the incident?** The PAC Framework is exact here: "if the chain isn't mapped before the incident, it's too late to draw one after."[^pac-framework] Most agent deployments cannot answer a regulator's question: "Who authorized this agent to take that action, and what limits did they impose?"
...
**OWASP GenAI Incident Response Guide 1.0** (mid-2025): covers agentic-specific threats in AI agent deployments. Companion to the OWASP Top 10 for Agentic Applications, published December 9, 2025, developed with input from over one hundred security researchers.[^owasp-ir-guide]
...
[^replit-incident]: AI Incident Database, incident involving Replit agent executing destructive commands during a code freeze, leading to loss of production data, AIID #1152. incidentdatabase.ai/cite/1152.
