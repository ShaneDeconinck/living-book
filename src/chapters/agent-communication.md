...
The Sigstore project extended Agent Card signing further with sigstore-a2a: a library for keyless signing of Agent Cards using Sigstore's infrastructure and SLSA provenance attestations.[^sigstore-a2a] In CI/CD environments, ambient OIDC credentials authenticate with Sigstore's certificate authority (Fulcio), which issues a short-lived certificate used to sign the Agent Card. The signature is recorded in Sigstore's transparency log (Rekor), creating a permanent, tamper-evident record. SLSA provenance attestations link each Agent Card to its source repository, commit SHA, and build workflow. This bridges agent identity and software supply chain trust: a receiving agent can verify not just that a card is authentic but where the agent code came from and how it was built. The approach is designed to eliminate long-lived signing keys, which would remove the key management burden that discourages adoption of card signing in practice.[^sigstore-a2a-hedge]
...
MCP-I (Model Context Protocol: Identity) is the most direct attempt to close the identity gap at the protocol layer.[^mcp-i] Vouched donated the specification to DIF in March 2026, where it is being developed under a dedicated MCP-I task force inside a new Trusted AI Agents Working Group. MCP-I extends MCP with four identity dimensions: cryptographically verifiable agent identity via DIDs, proof of principal authorization, delegated scope expressed as Verifiable Credentials, and trustworthiness signaling. Three conformance tiers structure adoption. The DIF enterprise member report that preceded MCP-I's formal standardization identified the same root problem this book opens with: current systems conflate authentication with authorization. MCP-I is a standards-track attempt to separate them using the DID and VC infrastructure that the identity chapter covers. If it achieves working group adoption at DIF, it becomes the bridge between MCP's communication model and the trust infrastructure the book argues agents require.
...
[^sigstore-a2a]: Sigstore, sigstore-a2a, github.com/sigstore/sigstore-a2a. Python library and CLI for keyless signing of A2A Agent Cards using Sigstore and SLSA provenance attestations. Also: Luke Hinds, "Building Trust in the AI Agent Economy: Sigstore Meets Agent2Agent," dev.to, July 2025.
[^sigstore-a2a-hedge]: sigstore-a2a is an early-stage library. Production adoption at scale has not been documented as of March 2026. The design claims are credible given Sigstore's established use in software supply chain signing, but this specific application to Agent Cards remains prototype territory.
[^mcp-i]: Decentralized Identity Foundation, "Why We Brought MCP-I to DIF (and Why DIF Said Yes)," blog.identity.foundation, March 5, 2026. Vouched donated MCP-I to DIF for open standardization under a dedicated task force inside the Trusted AI Agents Working Group. Four identity dimensions: cryptographically verifiable agent identity (DIDs), proof of principal authorization, delegated scope (Verifiable Credentials), trustworthiness signaling. Three conformance tiers for progressive adoption. See also DIF, "Digital Identity for Agentic Systems," blog.identity.foundation, March 12, 2026, for the enterprise use-case report that identifies authentication/authorization conflation as the root problem.
