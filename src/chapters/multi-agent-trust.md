...
A single agent requires the inverse of human trust: restricted to what it can do, not what it cannot.[^7] A multi-agent system requires trust inversion at every boundary, with the additional constraint that the trust envelope must be verifiable end-to-end across agents that may not share infrastructure, identity providers, or even organizational affiliation.
