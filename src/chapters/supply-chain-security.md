...
For agents, an AI-BOM needs to enumerate components that neither standard fully addresses yet:
...
Isolation reduces the blast radius of any single compromised component:
...
The agent supply chain is the newest and least mature layer of trust infrastructure. Every other chapter assumes the components inside the agent are trustworthy. That assumption must be verified, continuously, for every dependency in the chain.
