...
**Cascade severance.** In multi-agent systems, a governance failure in one agent propagates through delegation chains before the human can intervene. By the time the human detects the issue, the downstream effects are already in motion. This connects directly to multi-agent failure research: Galileo's analysis found that a single compromised agent could rapidly poison downstream decisions across a multi-agent chain, though the findings come from the company's own analysis rather than peer-reviewed research[^cascade-severance].
