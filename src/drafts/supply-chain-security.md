...
ClawHavoc was a coordinated campaign: one actor, 335 skills, identifiable patterns. Snyk's ToxicSkills study revealed a more systemic problem.[^toxicskills] Scanning 3,984 skills from ClawHub and skills.sh, Snyk found that 36.82% (1,467 skills) had at least one security flaw. 13.4% (534 skills) contained critical-level issues: malware distribution, prompt injection, exposed secrets. Seventy-six confirmed malicious payloads were identified through human-in-the-loop review, with eight still publicly available on clawhub.ai at the time of publication. The overlap between attack techniques was near-total: 100% of confirmed malicious skills contained malicious code patterns, and 91% simultaneously employed prompt injection. Hardcoded secrets appeared in roughly one-tenth of all skills and one-third of confirmed malicious samples. The barrier to publishing a skill on ClawHub was a SKILL.md file and a one-week-old GitHub account: no code signing, no security review, no sandbox by default. You do not need a coordinated campaign when a third of the ecosystem ships with vulnerabilities.
...
[^toxicskills]: Snyk, "ToxicSkills: Malicious AI Agent Skills in ClawHub," snyk.io, February 5, 2026. Authors: Luca Beurer-Kellner, Aleksei Kudrinskii, Marco Milanta, Kristian Bonde Nielsen, Hemang Sarkar, Liran Tal. Scan of 3,984 skills from ClawHub and skills.sh. Detection engine: mcp-scan (deterministic rules + multi-model analysis), 90-100% recall on confirmed malicious samples, 0% false-positive rate on legitimate curated samples.
...
Between January and February 2026, security researchers documented 30 CVEs against MCP infrastructure in just 60 days. The breakdown reveals a systemic pattern: 43% (13 CVEs) were exec()/shell injection, including CVE-2025-68144 in mcp-server-git, Anthropic's own official Git server. 20% (6 CVEs) hit tooling and infrastructure layers. 13% (4 CVEs) were authentication bypass. 10% (3 CVEs) were path traversal. The remaining 7% represented new attack classes that did not exist in the initial MCP threat model, including eval() injection (CVE-2026-1977, where a data visualization specification becomes code execution) and environment variable injection.[^mcp-30-cves]
...
The kill chain: create an issue (no special permissions needed) → developer opens Codespace → Copilot silently follows hidden instructions → token exfiltration → repository takeover. The agent does exactly what it was instructed to do.
...
[^anthropic-rsp]: Anthropic, "Responsible Scaling Policy Version 3.0," anthropic.com/responsible-scaling-policy/rsp-v3-0, effective February 24, 2026. ASL thresholds remain fixed; introduces public Frontier Safety Roadmap (non-binding goals) and mandatory Risk Reports every 3-6 months.
...
Supply chain security for agents is a layered approach that maps to different points in the dependency chain.
...
1. **Inventory your agent dependencies.** Generate an AI-BOM that covers not just code dependencies but model versions, MCP server connections, tool registrations, and context sources.
...
[^sigstore-a2a-sc]: Sigstore, sigstore-a2a, github.com/sigstore/sigstore-a2a. Python library for keyless signing of A2A Agent Cards using Sigstore infrastructure and SLSA provenance attestations. Created by Luke Hinds (former Security Engineering Lead (OCTO) at Red Hat, Sigstore creator). Uses ambient OIDC credentials in CI/CD, Fulcio certificate authority, Rekor transparency log. Links Agent Cards to source repositories, commit SHAs, and build workflows. See also: Luke Hinds, "Building Trust in the AI Agent Economy: Sigstore Meets Agent2Agent," dev.to, July 2025.
