...
SecurityScorecard's STRIKE Team mapped the OpenClaw exposure surface: approximately 42,900 unique IPs hosting exposed control panels across 82 countries, with 15,200 flagged as vulnerable to remote code execution (35.4% of observed deployments) and 53,300 correlating with prior breach activity in SecurityScorecard's threat intelligence data.[^securityscorecard] Shane noted the core lesson in his analysis of the OpenClaw chaos: "if the creator telling users not to do something doesn't work, documentation is not a security model."[^shane-openclaw] Rich context (OpenClaw's SOUL.md file) made the agent compelling. Missing access controls made it dangerous. Both layers are needed. But the supply chain dimension adds a third: you also need to trust the components you are loading into that context.
...
[^securityscorecard]: SecurityScorecard STRIKE Team, "How Exposed OpenClaw Deployments Turn Agentic AI Into an Attack Surface" and "Beyond the Hype: Moltbot's Real Risk Is Exposed Infrastructure, Not AI Superintelligence," securityscorecard.com, February 2026. Figures from the detailed analysis: 42.9K unique IPs across 82 countries, 15.2K vulnerable to RCE (35.4% of observed deployments), 53.3K correlated with prior breach activity via SecurityScorecard intelligence.
