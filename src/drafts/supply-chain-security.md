...
**Model Provider Trust.** Training data is not the only model supply chain risk. The model provider's safety commitments are themselves a dependency, and dependencies can change. In January 2026, Anthropic released Responsible Scaling Policy 3.0, shifting from absolute safety commitments to relative ones: safety measures now conditioned on competitor behavior rather than maintained regardless of what other labs do.[^anthropic-rsp] The company that many enterprise buyers trusted specifically *because* it held the line on safety redefined what holding the line means. Weeks later, the U.S. Department of Defense designated Anthropic a "supply-chain risk," but for the opposite reason: Anthropic refused to allow Claude's use for mass surveillance or fully autonomous weapons. The Pentagon contracted with OpenAI instead.[^dod-anthropic] Anthropic sued in March 2026, with over 30 employees from OpenAI and Google DeepMind filing an amicus brief in its defense.
...
QUIETVAULT is not an isolated case. Google identified five AI-powered malware families actively deployed in the wild, each exploiting AI capabilities differently: PROMPTFLUX rewrites its own source code hourly using Gemini to evade detection. PROMPTSTEAL, used by APT28 (Russia's GRU military intelligence), queries LLMs to generate credential-theft commands targeting Ukrainian systems. PROMPTLOCK is ransomware that uses LLMs to dynamically generate malicious Lua scripts at runtime. FRUITSHELL includes hardcoded prompts designed to bypass LLM-powered security analysis.[^google-ai-malware] The pattern across all five families: adversaries are not just targeting AI tools. They are using them. AI tools are simultaneously the asset to protect and the weapon being wielded against you. The defensive implication is direct: organizations need to monitor AI tool activity on developer machines with the same scrutiny they apply to administrative shells, and AI tools need structural containment (the [Sandboxing and Execution Security](execution-security.md) chapter's argument) that cannot be bypassed by permission flags intended for legitimate automation workflows.
...
[^anthropic-rsp]: Anthropic, Responsible Scaling Policy 3.0, January 2026. Shifted from absolute safety commitments (risk thresholds maintained regardless of competitor actions) to relative commitments conditioned on competitor behavior. Help Net Security, "Does Anthropic deserve the trust of the cybersecurity community?" March 12, 2026.
