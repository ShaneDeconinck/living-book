...
MCP-I (Model Context Protocol — Identity), donated to the Decentralized Identity Foundation in March 2026, is the most direct attempt to close the identity gap at the protocol layer.[^mcp-i] Vouched donated the specification to DIF, where it is being developed under a dedicated MCP-I task force inside a new Trusted AI Agents Working Group. MCP-I extends MCP with four identity dimensions: cryptographically verifiable agent identity via DIDs, proof of principal authorization, delegated scope expressed as Verifiable Credentials, and trustworthiness signaling. Three conformance tiers define adoption levels. The DIF enterprise member report that preceded MCP-I's formal standardization identified the same root problem this book opens with: current systems conflate authentication with authorization. MCP-I is a standards-track attempt to separate them, at the protocol level, using the DID and VC infrastructure that the identity chapter covers. If it achieves working group adoption at DIF, it becomes the bridge between MCP's communication model and the trust infrastructure the book argues agents require.
