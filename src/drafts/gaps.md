...
MCP's OAuth implementation is itself an attack surface, distinct from tool description poisoning. Two CVEs in ha-mcp (a Home Assistant MCP server) illustrate the pattern. CVE-2026-32112 (CVSS 6.8): the OAuth consent form renders user-controlled parameters via Python f-strings with no HTML escaping, enabling XSS that can exfiltrate Long-Lived Access Tokens.[^ha-mcp-xss] CVE-2026-32111 (CVSS 5.3): the same server accepts user-supplied URLs via open Dynamic Client Registration and makes server-side requests without validation, enabling SSRF for internal network reconnaissance.[^ha-mcp-ssrf] Both affect the OAuth beta mode introduced to comply with MCP's 2025-11-25 authorization spec. The pattern mirrors CVE-2026-26118 (Azure MCP SSRF): implementing OAuth in MCP servers introduces the full web application vulnerability surface into what was previously a local stdio process.
