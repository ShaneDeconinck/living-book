...
**A2A (Agent-to-Agent Protocol)** handles agent-to-agent discovery and communication. Google's protocol reached version 0.3 with gRPC support, agent card signing for security, and SDKs across Python, Go, JavaScript, Java, and .NET. Over 150 organizations now support A2A, including Atlassian, Salesforce, SAP, PayPal, Microsoft, and Amazon. Auth0 is partnering with Google Cloud to define A2A authentication specifications.[^8]
